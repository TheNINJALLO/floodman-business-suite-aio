...
if [ "$config_hash" != "$old_hash" ] || [ ! -s "$runtime_web/index.html" ]; then
  fm_log "Preparing the branded Gauzy browser bundle..."
  rm -rf "$runtime_web"
  mkdir -p "$runtime_web"
  cp -a /opt/gauzy-web-pristine/. "$runtime_web/"
  cd "$runtime_web"
  envsubst < replacements.sed > replacements_values.sed
  # Gauzy's own Compose entrypoint replaces the generated Docker placeholders in root JS bundles.
  for file in ./*.js; do
    [ -f "$file" ] || continue
...
fm_log "Starting Floodman Gauzy Hub on port $SERVER_PORT..."
