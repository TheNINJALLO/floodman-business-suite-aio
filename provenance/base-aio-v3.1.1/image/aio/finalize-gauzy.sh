...
fm_log "Waiting for Gauzy API and minimum platform seed..."
fm_wait_http "http://127.0.0.1:3000/api/auth/authenticated" 2700 true || fm_die "Gauzy API did not become ready within 45 minutes."
...
until $PSQL -Atc "SELECT CASE WHEN to_regclass('public.\"user\"') IS NOT NULL AND to_regclass('public.\"organization\"') IS NOT NULL AND to_regclass('public.\"employee\"') IS NOT NULL THEN 1 ELSE 0 END" 2>/dev/null | grep -q '^1$'; do
  tries=$((tries + 1)); [ "$tries" -lt 180 ] || fm_die "Gauzy tables did not become available."
  sleep 10
done

installed="$($PSQL -Atc "SELECT owner_email FROM floodman_installation_meta WHERE singleton=true" 2>/dev/null || true)"
if [ -z "$installed" ]; then
  fm_log "Applying Floodman company and Owner profile to Gauzy..."
  $PSQL \
...
VERIFY
  ...
  fm_log "Clean Gauzy ERP verified: owner=1 contacts=0 invoices=0 payments=0"
else
  ...
  [ "$current_owner" = 1 ] || fm_die "Configured Owner no longer matches the initialized Gauzy database. Restore the original Pterodactyl Owner email variable."
  fm_log "Existing Floodman Gauzy installation verified for $installed."
fi
