...
log() { printf '[Floodman Mobile v4.6.8] %s\n' "$*"; }
die() { printf '[Floodman Mobile v4.6.8] ERROR: %s\n' "$*" >&2; exit 1; }

base='/opt/floodman/aio'
hotfix='/home/container/runtime/floodman-v4.6.8'
...
overlay_zip="/home/container/floodman-operations-runtime-v4.6.8.zip"
overlay_extract="$hotfix/app-overlay"
overlay_root="$overlay_extract/floodman-operations-v4.6.8"
[ -r "$overlay_zip" ] || die 'Keep floodman-operations-runtime-v4.6.8.zip in the Pterodactyl server root before starting.'
rm -rf "$overlay_extract"
mkdir -p "$overlay_extract"
unzip -q "$overlay_zip" -d "$overlay_extract" || die 'Could not extract the Floodman v4.6.8 mobile-API overlay.'
[ "$(cat "$overlay_root/VERSION" 2>/dev/null || true)" = '4.6.8' ] || die 'The uploaded mobile-API overlay is not Floodman v4.6.8.'
( cd "$overlay_root" && sha256sum -c MANIFEST.sha256 >/dev/null ) || die 'The Floodman v4.6.8 mobile-API overlay failed its checksum verification.'
log 'Verified the cumulative Floodman v4.6.8 full-ERP login overlay with authentication-aware routing, injected browser recovery, separate desktop/mobile workspaces, RoomFlow workspaces, guarded PDFs, estimates, invoices, scheduling, and payments.'
...
printf '%s\n' '4.6.8' > /home/container/config/floodman-active-runtime.txt
...
overlay="$FM_HOME/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/competitor-intel"
[ -r "$overlay/app/main.py" ] || fm_die 'The Floodman Competitor Intelligence v4.6.8 overlay is missing.'
cd "$overlay"
...
(() => {
  'use strict';
  const release = 'pterodactyl-mobile-v4.6.8';
...
<body><main>
  ...
  <p class="muted" style="font-size:12px;margin-top:18px">Release pterodactyl-mobile-v4.6.8</p>
</main>
<script>
(() => {
...
overlay="$FM_HOME/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/office-console"
...
config_hash="$(printf '%s\n' "$SERVER_PORT|$FLOODMAN_COMPANY_NAME|pterodactyl-mobile-v4.6.8" | sha256sum | awk '{print $1}')"
...
envsubst '${HUB_TITLE} ${HUB_RELEASE} ${HUB_OFFICE_URL} ${HUB_ROOMFLOW_URL} ${HUB_DOCUMENSO_URL} ${HUB_MAILPIT_URL} ${HUB_ENGINEERING_URL} ${HUB_API_URL} ${HUB_COMPETITOR_URL} ${HUB_SYNC_STATUS_URL} ${HUB_REMOTE_ACCESS_ENABLED} ${HUB_REMOTE_DOCUMENSO_PORT} ${HUB_REMOTE_MAILPIT_PORT} ${HUB_REMOTE_ENGINEERING_PORT} ${HUB_REMOTE_API_PORT} ${HUB_REMOTE_DOCUMENSO_URL} ${HUB_REMOTE_MAILPIT_URL} ${HUB_REMOTE_ENGINEERING_URL} ${HUB_REMOTE_API_URL}' \
  < "$FM_HOME/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/hub/hub-config.js.template" \
  > "$FM_HOME/runtime/hub/floodman-hub-config.js"
envsubst '${SERVER_PORT} ${HUB_RELEASE}' \
  < "$FM_HOME/runtime/floodman-v4.6.8/nginx.conf.template" \
...
# Use the verified v4.6.8 Nginx template, which serves the manifest, service worker, install page, icons, and safe offline shell.
...
overlay="$FM_HOME/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/local-lab"
[ -r "$overlay/app/main.py" ] || fm_die 'The Floodman Engineering Sandbox v4.6.8 overlay is missing.'
cd "$overlay"
...
. /opt/floodman/aio/common.sh
while [ ! -f "$FM_RUN/databases-ready" ] || [ ! -f "$FM_RUN/gauzy-finalized" ]; do sleep 2; done
overlay="$FM_HOME/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/orchestrator"
[ -r "$overlay/app/main.py" ] || fm_die 'The Floodman API v4.6.8 overlay is missing.'
cd "$overlay"
...
sed \
  -e 's/pterodactyl-mobile-v[0-9][0-9.]*/pterodactyl-mobile-v4.6.8/g' \
...
grep -Fq '/home/container/runtime/floodman-v4.6.8/app-overlay/floodman-operations-v4.6.8/' "$hotfix/nginx.conf.template" || die 'The Hub is not serving the current v4.6.8 frontend assets.'
...
python3 -m py_compile \
  ...
  || die 'Could not validate the Floodman v4.6.8 runtime patches.'
if grep -RIn --include='*.py' -E '(^|[[:space:]])(from[[:space:]]+PIL|import[[:space:]]+PIL)' \
  ...
  die 'A Pillow/PIL import remains in the uploaded Floodman v4.6.8 runtime.'
fi
PYTHONPATH="/opt/pydeps/office-console:$overlay_root/office-console:/opt/floodman/office-console" \
  ...
  || { cat /home/container/logs/floodman-office-overlay-preflight.log >&2 || true; die 'The exact v4.6.8 Office overlay could not be imported during startup preflight.'; }
PYTHONPATH="/opt/pydeps/office-console:$overlay_root/office-console:/opt/floodman/office-console" \
...
grep -Fq '$FM_HOME/runtime/floodman-v4.6.8/nginx.conf.template' "$hotfix/start-hub.sh" || die 'The Floodman Hub is not loading the gateway-enabled Nginx template.'
if grep -Fq 'app-overlay/floodman-operations-v4.6.8/aio/nginx.conf.template' "$hotfix/start-hub.sh"; then
...
# Validate the full v4.6.8 native-app parity, RoomFlow migration, and scheduling additions.
...
# Remove only the generated ERP browser copy so v4.6.8 is guaranteed to
...
log "Floodman v4.6.8 validated the stable root selector, true desktop shell, service-aware online detection, separate mobile shell, authentication-aware /full-erp launcher and injected ERP boot guard, matched Android alpha11 runtime, RoomFlow workspaces, guarded PDFs, estimates, invoices, payments, calendar, and public API at $TAILSCALE_MAIN_URL..."
