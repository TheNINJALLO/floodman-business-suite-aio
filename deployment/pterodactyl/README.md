...
```text
Server:  Floodman Operations 4.6.8
...
Import `egg-floodman-operations-mobile-v4.6.8.json`, then upload the two current files from `deployment/releases/` to `/home/container`:

```text
mobile-start-v4.6.8.sh -> mobile-start.sh
floodman-operations-runtime-v4.6.8.zip (do not extract)
```
...
The v4.6.8 egg, launcher, ZIP, internal manifest, and deployment checksums are regenerated together by `scripts/package_pterodactyl_release.py` and checked by `scripts/verify_pterodactyl_release.py`. Historical copies under `release-artifacts/` are rollback provenance, not the current upload source.
