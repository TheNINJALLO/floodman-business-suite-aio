...
The registry pattern this repo uses at the meta level is the same discipline applied to files: per `docs/ARCHITECTURE.md`, a 2,018-token `SKILL.md` routes to 415,028 tokens of references, and no single request pays for material it doesn't touch. An agent reading "just in case" is doing the thing the registry exists to prevent, one level down.
