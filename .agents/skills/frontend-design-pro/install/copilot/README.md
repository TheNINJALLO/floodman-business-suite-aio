...
**Degradation:** always-loaded instructions, not fetch-on-demand — Copilot cannot open `skills/{id}/references/*.md` mid-conversation, and there is not even ChatGPT's retrieval search to soften it. The 116 reference files reach the model only if you paste one in by hand.
