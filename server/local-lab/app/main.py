...
class LabState:
    ...
    def _initialize_empty(self, setup: dict[str, Any] | None = None) -> None:
        ...
        self.square_cards: dict[str, dict[str, Any]] = {}
        self.square_payments: dict[str, dict[str, Any]] = {}
    ...
    def _restore(self, value: dict[str, Any]) -> None:
        with self.lock:
            ...
            if not isinstance(getattr(self, "square_cards", None), dict):
                self.square_cards = {}
            if not isinstance(getattr(self, "square_payments", None), dict):
                self.square_payments = {}
...
# ---------------------------------------------------------------------------
# Floodman ERP compatibility mock
# ---------------------------------------------------------------------------
...
def square_customer_search(payload: dict[str, Any]) -> dict[str, Any]:
    filter_value = ((payload.get("query") or {}).get("filter") or {})
    exact_email = (filter_value.get("email_address") or {}).get("exact")
    exact_phone = (filter_value.get("phone_number") or {}).get("exact")
    values = list(state.square_customers.values())
    if exact_email:
        values = [item for item in values if item.get("email_address", "").lower() == str(exact_email).lower()]
    if exact_phone:
        digits = "".join(ch for ch in str(exact_phone) if ch.isdigit())[-10:]
        values = [item for item in values if "".join(ch for ch in str(item.get("phone_number") or "") if ch.isdigit())[-10:] == digits]
    return {"customers": values[: int(payload.get("limit") or 100)]}
...
@app.get("/square/v2/cards")
def square_cards_list(customer_id: str = "") -> dict[str, Any]:
    values = [dict(item) for item in state.square_cards.values()]
    if customer_id:
        values = [item for item in values if str(item.get("customer_id") or "") == customer_id]
    values = [item for item in values if bool(item.get("enabled", True))]
    return {"cards": values}


@app.get("/square/v2/cards/{card_id}")
def square_card_get(card_id: str) -> dict[str, Any]:
    card = state.square_cards.get(card_id)
    if not card:
        raise HTTPException(404, "card not found")
    return {"card": card}


@app.post("/square/v2/cards/{card_id}/disable")
def square_card_disable(card_id: str) -> dict[str, Any]:
    card = state.square_cards.get(card_id)
    if not card:
        raise HTTPException(404, "card not found")
    card["enabled"] = False
    card["disabled_at"] = datetime.now(UTC).isoformat()
    state.save()
    return {"card": card}


def _save_mock_card(customer_id: str) -> dict[str, Any]:
    for item in state.square_cards.values():
        if str(item.get("customer_id") or "") == customer_id and bool(item.get("enabled", True)):
            return item
    card_id = state.next_id("card")
    card = {
        "id": card_id,
        ...
        "last_4": "1111",
        "exp_month": 12,
        "exp_year": datetime.now(UTC).year + 3,
        "card_type": "CREDIT",
        "prepaid_type": "NOT_PREPAID",
        "enabled": True,
        "reference_id": f"local-card-{customer_id}",
        "created_at": datetime.now(UTC).isoformat(),
    }
    state.square_cards[card_id] = card
    state.save()
    return card
...
async def square_invoice_publish(invoice_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    invoice["created_at"] = invoice.get("created_at") or datetime.now(UTC).isoformat()
    invoice["updated_at"] = datetime.now(UTC).isoformat()
    _refresh_square_invoice(invoice)

    automatic_requests = [
        item for item in invoice.get("payment_requests") or []
        if str(item.get("automatic_payment_source") or "").upper() == "CARD_ON_FILE"
    ]
    if automatic_requests:
        customer_id = str((invoice.get("primary_recipient") or {}).get("customer_id") or "")
        for request in automatic_requests:
            card_id = str(request.get("card_id") or "")
            card = state.square_cards.get(card_id)
            if not card or not bool(card.get("enabled", True)) or str(card.get("customer_id") or "") != customer_id:
                request["automatic_payment_source"] = "NONE"
                invoice["automatic_payment_error"] = "The selected card on file is unavailable."
                continue
            computed = int((request.get("computed_amount_money") or {}).get("amount") or 0)
            request.setdefault("total_completed_amount_money", {"amount": 0, "currency": "USD"})
            request["total_completed_amount_money"]["amount"] = computed
            payment_id = state.next_id("spayment")
            state.square_payments[payment_id] = {
                "id": payment_id,
                "invoice_id": invoice_id,
                "customer_id": customer_id,
                "card_id": card_id,
                "amount_money": dict(request.get("computed_amount_money") or {}),
                "status": "COMPLETED",
                "created_at": datetime.now(UTC).isoformat(),
            }
        _refresh_square_invoice(invoice)
...
def square_payment_page(invoice_id: str) -> HTMLResponse:
    ...
    allow_save = bool(invoice.get("store_payment_method_enabled"))
    save_control = ""
    if allow_save:
        save_control = "<label style='display:block;margin:18px 0'><input type='checkbox' name='save_card' value='yes'> Save this card securely with Square for future Floodman invoices</label>"
    return HTMLResponse(
        f"<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>Floodman payment</title></head>"
        f"<body style='font-family:Arial;max-width:580px;margin:40px auto;padding:18px'><h1>Floodman payment sandbox</h1>"
        f"<p>Invoice <b>{html.escape(str(invoice.get('invoice_number') or invoice_id))}</b></p>"
        f"<p>Remaining: <b>${remaining/100:,.2f}</b></p>"
        f"<div style='background:#fff3cd;padding:12px;border-radius:8px'>Testing only. No real card number is collected. The sandbox creates a tokenized test card ending in 1111 only when the customer checks the consent box.</div>"
        f"<form method='post' action='/lab/invoices/{invoice_id}/pay'>{save_control}<button style='padding:12px 18px'>Pay remaining balance</button></form>"
        f"<p><a href='/app'>Return to Floodman Operations</a></p></body></html>"
    )


@app.post("/lab/invoices/{invoice_id}/pay")
async def lab_pay_invoice(
    invoice_id: str,
    amount_cents: int | None = Form(default=None),
    save_card: str = Form(default=""),
) -> RedirectResponse:
    invoice = state.square_invoices.get(invoice_id)
    if not invoice:
        raise HTTPException(404, "invoice not found")
    ...
    customer_id = str((invoice.get("primary_recipient") or {}).get("customer_id") or "")
    saved_card = None
    if save_card == "yes" and invoice.get("store_payment_method_enabled") and customer_id:
        saved_card = _save_mock_card(customer_id)
    payment_id = state.next_id("spayment")
    state.square_payments[payment_id] = {
        "id": payment_id,
        "invoice_id": invoice_id,
        "customer_id": customer_id,
        "card_id": (saved_card or {}).get("id"),
    ...
    invoice["updated_at"] = datetime.now(UTC).isoformat()
    _refresh_square_invoice(invoice)
    state.last_action = f"Paid ${amount/100:,.2f} on {invoice_id}" + (" and saved a Square test card" if saved_card else "")
    state.save()
    await _post_square_event(invoice_id)
...
async def api_state() -> dict[str, Any]:
    return {
        ...
        "square_cards": list(state.square_cards.values()),
        "square_payments": list(state.square_payments.values()),
