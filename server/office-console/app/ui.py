...
PWA_BODY = "<script defer src='/floodman-pwa.js?release=4.6.10'></script>"
WORKSPACE_CSS = "<link rel='stylesheet' href='/floodman-workspace.css?release=4.6.10'>"
...
NAV_GROUPS = [
    ("Command", [
        ("/office/desktop", "Desktop Workspace", "dashboard", "dashboard.view"),
...
MOBILE_PRIMARY = [
    ("/office/mobile?mobile=1", "Home", "⌂", "mobile", "dashboard.view"),
...
(() => {
  ...
  const desktopShell = () => document.documentElement.dataset.workspace === 'desktop' || (document.documentElement.dataset.workspace !== 'mobile' && desktop.matches);
  const updateMode = () => {
    if (desktopShell()) setOpen(false);
    sidebar.setAttribute('aria-hidden', desktopShell() ? 'false' : sidebar.classList.contains('is-open') ? 'false' : 'true');
  };
  if (desktop.addEventListener) desktop.addEventListener('change', updateMode); else desktop.addListener(updateMode);
  updateMode();
  const setWorkspaceMode = (mode) => { try { localStorage.setItem('floodmanWorkspaceMode', mode); if (mode === 'desktop') localStorage.setItem('floodmanDesktopMode', '1'); else localStorage.removeItem('floodmanDesktopMode'); document.documentElement.dataset.workspace = mode; } catch (_) {} };
...
.roomflow-workspace-card{padding:0;overflow:hidden}.roomflow-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px 15px;border-bottom:1px solid var(--line);background:#0b1b2e}.roomflow-toolbar-copy b,.roomflow-toolbar-copy small{display:block}.roomflow-toolbar-copy small{margin-top:3px;color:var(--muted)}.roomflow-frame{display:block;width:100%;height:calc(100dvh - 185px);min-height:720px;border:0;background:#07111f}.roomflow-history-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px}.roomflow-history-card{padding:13px;border:1px solid var(--line);border-radius:12px;background:#0b192b}.roomflow-history-card b,.roomflow-history-card small{display:block}.roomflow-history-card small{margin-top:5px;color:var(--muted)}
@media(max-width:1100px){.roomflow-workspace-card{margin-left:-12px;margin-right:-12px;border-left:0;border-right:0;border-radius:0}.roomflow-toolbar{align-items:stretch;flex-direction:column}.roomflow-toolbar .actions{display:grid;grid-template-columns:1fr 1fr}.roomflow-toolbar .button{width:100%}.roomflow-frame{height:calc(100dvh - 236px);min-height:620px}}
@media(max-width:560px){.roomflow-toolbar .actions{grid-template-columns:1fr}.roomflow-frame{height:calc(100dvh - 280px);min-height:560px}}
...
def simple_page(title: str, body: str) -> str:
    return f"""<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,viewport-fit=cover'><meta name='theme-color' content='#081426'><title>{esc(title)} · Floodman Operations</title><meta name='author' content='Josh Aldrich'><meta name='application-name' content='Floodman Operations'>{WORKSPACE_HEAD}{PWA_HEAD}<style>{BASE_CSS}</style>{WORKSPACE_CSS}</head><body><div class='auth-shell'><div class='auth-card'><div class='brand big'>Floodman Operations</div><div class='subbrand'>Created by Josh Aldrich</div>{body}</div></div>{PWA_BODY}</body></html>"""
