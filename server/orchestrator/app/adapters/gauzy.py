...
class GauzyClient:
    """Internal Floodman ERP compatibility adapter for contacts, estimates, invoices, and payments."""
    ...
    def _context(self) -> tuple[str, str, str]:
        ...
        if not tenant_id or not organization_id:
            raise ProviderError(
                "gauzy",
                "could not discover the Floodman ERP workspace and organization; open Floodman Operations, create or select the Floodman organization, then retry",
                details={
                    "tenant_id": tenant_id,
    ...
    def ensure_property_project(
        ...
        *,
        ...
        """Create one Floodman ERP project per RoomFlow job or property when the API permits it.

        Floodman ERP projects give the service property a native home for tasks, employee time,
        invoice items, payments, expenses, and reports. The workflow remains usable if a
        Floodman ERP deployment disables project creation; callers may continue with a contact-only
    ...
    def find_estimate(self, invoice_number: int) -> dict[str, Any] | None:
        # Floodman ERP supports filtering the invoice collection. We still verify every returned
    ...
    def promote_estimate_to_invoice(self, estimate_id: str, *, note: str) -> dict[str, Any]:
        """Accept and convert the Floodman ERP estimate into the invoice ledger record.

        Floodman ERP models estimates and invoices with one entity. This keeps the identifier stable,
    ...
    def issue_final_invoice(self, invoice_id: str, *, due_date: str, note: str) -> dict[str, Any]:
        """Mark the Floodman ERP invoice as sent and due upon receipt."""
    ...
    def generate_invoice_link(self, invoice_id: str) -> str | None:
        ...
        return f"{origin}/index.html?desktop=1#/share/invoices/{token}"
