...
class WorkflowService:
    def __init__(self, settings: Settings):
        ...
        self.office = FloodmanOfficeClient(settings)
    ...
    def handle(self, event: dict[str, Any]) -> None:
        handlers = {
            ...
            "ATTACH_CLIENT_FILE": self.attach_client_file,
    ...
    def sync_estimate_to_gauzy(self, job_id: str, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("Floodman ERP property project could not be created for job %s: %s", job_id, exc)

        estimate = self.gauzy.create_estimate(job, contact_id, project_id or None)
        estimate_id = str(estimate.get("id") or "")
        if not estimate_id:
            raise RuntimeError("Floodman ERP estimate response did not include id")
    ...
    def finalize_document(self, job_id: str, payload: dict[str, Any]) -> None:
        ...
        if document["kind"] == "WORK_AUTHORIZATION":
            if not job.get("gauzy_estimate_id"):
                raise PermanentEventError("Work Authorization completed before the Floodman ERP estimate was mapped")
        ...
        with transaction() as conn:
            ...
            # Client-file attachment has its own retryable event so a temporary
            # Office outage can never roll back a valid completed signature.
            enqueue(conn, job_id, "ATTACH_CLIENT_FILE", {"document_id": document_id})
            audit(
                conn,
                current["organization_id"],
                "DOCUMENSO",
    ...
    def attach_client_file(self, job_id: str, payload: dict[str, Any]) -> None:
        document_id = str(payload.get("document_id") or "")
        if not document_id:
            raise PermanentEventError("ATTACH_CLIENT_FILE event is missing document_id")
        with transaction() as conn:
            document = get_document(conn, document_id)
            job = get_job(conn, job_id)
        if document.get("status") != "COMPLETED":
            raise RuntimeError(f"Signed document {document_id} is not completed yet")
        stored_path = str(document.get("stored_path") or "")
        signed_hash = str(document.get("signed_pdf_sha256") or "")
        if not stored_path or not signed_hash:
            raise PermanentEventError("Completed document does not contain its immutable PDF path and hash")

        titles = {
            "WORK_AUTHORIZATION": "Signed Work Authorization",
            "CHANGE_ORDER": "Signed Change Order",
            "COMPLETION_OF_SERVICE": "Signed Completion of Service",
        }
        result = self.office.attach_client_file(
            {
                ...
                "revision": int(document["revision"]),
                "status": "SIGNED",
                "title": titles.get(document["kind"], document["kind"].replace("_", " ").title()),
                "stored_path": stored_path,
                "signed_pdf_sha256": signed_hash,
                ...
                "customer": dict(job.get("customer") or {}),
                "property": dict(job.get("property") or {}),
                "floodman_contact_id": job.get("gauzy_contact_id"),
                "floodman_project_id": job.get("gauzy_project_id"),
                "floodman_estimate_id": job.get("gauzy_estimate_id"),
                "floodman_invoice_id": job.get("gauzy_invoice_id"),
            }
        )
        client_file_id = str(result.get("client_file_id") or document_id)
        with transaction() as conn:
            save_mapping(
                conn,
                "floodman-office",
            ...
            audit(
                ...
                "SYSTEM",
                None,
                "SIGNED_DOCUMENT_ATTACHED_TO_CLIENT",
                "workflow_document",
                document_id,
                {
    ...
    def create_change_order_invoice(self, job_id: str, payload: dict[str, Any]) -> None:
        ...
        if not job.get("gauzy_invoice_id"):
            raise PermanentEventError("Change Order cannot update an unmapped Floodman ERP invoice")
        ...
        # then update the Floodman ERP ledger, so an application retry cannot duplicate line items.
    ...
    def create_final_invoice(self, job_id: str, payload: dict[str, Any]) -> None:
        ...
        if job.get("final_invoice_issued_at"):
            return
        if not job.get("gauzy_invoice_id"):
            raise PermanentEventError("Final invoice cannot be issued before the Floodman ERP invoice mapping exists")
    ...
    def process_square_event(self, job_id: str, payload: dict[str, Any]) -> None:
        ...
        if not job.get("gauzy_invoice_id") or not job.get("gauzy_contact_id"):
            raise PermanentEventError("Square payment cannot be synchronized before Floodman ERP mappings exist")
        while True:
            with transaction() as conn:
