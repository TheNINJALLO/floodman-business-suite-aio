(() => {
  ...
  function openModal() {
    const values = inferred(); const modal = $('#fmrf-modal'); modalReturnFocus = document.activeElement; modal.inert = false; modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); document.documentElement.classList.add('fmrf-modal-open');
  ...
  function closeModal() { const modal=$('#fmrf-modal'); modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); modal.inert = true; document.documentElement.classList.remove('fmrf-modal-open'); if (modalReturnFocus?.focus) modalReturnFocus.focus(); }
  ...
  function install() {
    ...
    const modal=document.createElement('div'); modal.id='fmrf-modal'; modal.className='fmrf-modal-backdrop'; modal.setAttribute('aria-hidden','true'); modal.inert = true;
    ...
    $('#fmrf-open').onclick=openModal; $('#fmrf-close').onclick=closeModal; $('#fmrf-close-icon').onclick=closeModal; modal.addEventListener('click', e => { if(e.target===modal) closeModal(); }); document.addEventListener('keydown', e => { if(e.key === 'Escape' && modal.classList.contains('open')) { closeModal(); return; } if(e.key !== 'Tab' || !modal.classList.contains('open')) return; const focusable=Array.from(modal.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(node=>!node.hidden&&node.getClientRects().length); if(!focusable.length)return; const first=focusable[0],last=focusable[focusable.length-1]; if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();} }); $('#fmrf-save').onclick=submit;
