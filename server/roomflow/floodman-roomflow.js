(() => {
  ...
  function openModal() {
    const values = inferred(); const modal = $('#fmrf-modal'); modal.classList.add('open'); modal.setAttribute('aria-hidden','false');
  ...
  function closeModal() { const modal=$('#fmrf-modal'); modal.classList.remove('open'); modal.setAttribute('aria-hidden','true'); }
  ...
  function install() {
    ...
    $('#fmrf-open').onclick=openModal; $('#fmrf-close').onclick=closeModal; modal.addEventListener('click', e => { if(e.target===modal) closeModal(); }); $('#fmrf-save').onclick=submit;
