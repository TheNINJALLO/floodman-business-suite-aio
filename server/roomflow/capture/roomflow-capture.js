(() => {
  ...
  async function replayOutbox() {
    const jobId = activeJobId(); if (!jobId || !navigator.onLine) return;
  ...
  const bridge = window.RoomFlowCaptureBridgeV2 = {
    ...
    request(action, payload = {}, timeoutMs = 120000) {
      ...
      const envelope = { bridgeVersion: 2, requestId, action, payload: clone(payload) };
      return new Promise((resolve, reject) => {
        ...
        try {
          if (window.FloodmanNative?.roomFlowCaptureV2) window.FloodmanNative.roomFlowCaptureV2(JSON.stringify(envelope));
    ...
    receive(value) {
      ...
      if (!envelope || envelope.bridgeVersion !== 2 || !envelope.requestId) return false;
      if (envelope.event === 'progress') { window.RoomFlowCapture?.receiveProgress(envelope); return true; }
      ...
      if (envelope.ok) pending.resolve(envelope.result || {}); else pending.reject(new Error(envelope.error?.message || 'Device capture failed.'));
      return true;
    }
  };
  ...
  async function loadCapabilities() {
    try {
      const value = await bridge.request('capabilities', {}, 5000);
  ...
  function updateHistory(vertices) { model.history.apply(vertices); model.room.vertices = normalizedVertices(model.history.value(), 'manual-corrected'); model.room.scanMetadata.manualCorrectionCount += 1; boundsRoom(model.room); }
  ...
  function buildManualRoom(form) {
    ...
    model.room = boundsRoom(room); model.history.reset(model.room.vertices); model.step = 'review';
  }
  ...
  async function startNativeCapture() {
    ...
    try {
      ...
      model.room = normalizeNativeResult(result); model.history.reset(model.room.vertices); model.revision = 0; model.step = 'review'; setStatus('Device capture received. Review every measurement before saving.', 'good');
  ...
  async function cancelNativeCapture() {
    try { await bridge.request('cancelCapture', { requestId: model.nativeRequestId }, 5000); } catch (_) { }
  ...
  async function saveRoom() {
    ...
    try {
      const result = await fetchJson(path, { method: model.revision ? 'PUT' : 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(operation) });
      model.revision = result.revision; model.room = result.room; applyRoomToUpstream(model.room); await loadRooms(); model.step = 'saved'; setStatus('Room saved to this Floodman job.', 'good');
    } catch (error) {
      if (error.code === 'CONFLICT') { setStatus('This room changed on another device. Reload it before applying your edits.', 'bad'); }
      else if (!navigator.onLine || error instanceof TypeError) { queueOperation(operation); applyRoomToUpstream(model.room); model.step = 'saved'; setStatus('No connection. The room is saved on this device and will synchronize automatically.', 'warn'); }
  ...
  async function loadRooms() {
    ...
    try { const data = await fetchJson(`${API}/jobs/${encodeURIComponent(jobId)}/capture/rooms`); model.rooms = data.items || []; }
    catch (error) { if (error.status !== 404) setStatus(error.message, 'bad'); }
  }
  ...
  async function deleteRoom(index) {
    ...
    try {
      ...
      window.autosaveJob?.(); await loadRooms(); setStatus('Room removed from this job.', 'good'); render();
    } catch (error) {
  ...
  function editRoom(index) {
    ...
    model.room = clone(record.room); model.revision = number(record.revision); model.history.reset(model.room.vertices); model.step = 'review'; render();
  }

  function deleteOpening(index) { model.room.openings.splice(index, 1); model.room.scanMetadata.manualCorrectionCount += 1; boundsRoom(model.room); render(); }
  function addOpening(form) {
    ...
    model.room.scanMetadata.manualCorrectionCount += 1; boundsRoom(model.room);
  }

  function affectedFromForm(form) {
    const data = new FormData(form); const values = [];
    if (data.get('floor')) values.push({ surface: 'floor', scope: 'entire-surface', percent: 100 });
    if (data.get('ceiling')) values.push({ surface: 'ceiling', scope: 'entire-surface', percent: 100 });
    for (const value of data.getAll('wall')) values.push({ surface: 'wall', scope: 'entire-wall', wallSegmentIndex: number(value), percent: 100 });
    model.room.affectedAreas = values;
  }
  ...
  function planSvg(room) {
    ...
    return `<svg viewBox="0 0 500 280" role="img" aria-label="Reviewed floor plan for ${escapeHtml(room.name)}"><polygon points="${values.map(xy).join(' ')}"></polygon>${values.map(value => { const [x, y] = xy(value).split(','); return `<circle cx="${x}" cy="${y}" r="5"></circle>`; }).join('')}</svg>`;
  }
  ...
  function setupHtml() {
    ...
    return `<div class="fm-capture-grid">
      ...
      <section class="fm-capture-card soft fm-capture-wide"><h3>Add a room</h3><p>Choose device scanning when available, or enter dimensions manually. Both paths use the same review screen.</p><div class="fm-capture-actions"><button class="fm-capture-button" type="button" data-capture-action="scan" ${!nativeAvailable || !activeJobId() ? 'disabled' : ''}>Scan room with this device</button><button class="fm-capture-button secondary" type="button" data-capture-action="manual" ${!activeJobId() ? 'disabled' : ''}>Enter room manually</button></div><p>${nativeAvailable ? `Available: ${escapeHtml(model.capabilities.preferredMode)}.` : 'Camera measurement is not available in this browser. Manual entry is always available.'}</p></section>
  ...
  function affectedHtml() {
    ...
    return `<form id="fm-capture-affected-form" class="fm-capture-grid">
      <section class="fm-capture-card"><h3>Select affected surfaces</h3><p>Choose only what belongs in this loss scope. These selections drive estimate and work-order quantities.</p><div class="fm-capture-checks"><label class="fm-capture-check"><input type="checkbox" name="floor" ${selected.has('floor:') ? 'checked' : ''}> Entire floor</label><label class="fm-capture-check"><input type="checkbox" name="ceiling" ${selected.has('ceiling:') ? 'checked' : ''}> Entire ceiling</label>${lengths.map((length, index) => `<label class="fm-capture-check"><input type="checkbox" name="wall" value="${index}" ${selected.has(`wall:${index}`) ? 'checked' : ''}> Wall ${index + 1} · ${round(length)} ft</label>`).join('')}</div></section>
  ...
  function render() {
    ...
    const steps = ['setup', model.step === 'manual' || model.step === 'scanning' ? model.step : 'capture', 'review', 'affected'];
    const order = { setup: 0, manual: 1, scanning: 1, capture: 1, review: 2, affected: 3, saved: 4 }; const current = order[model.step] ?? 0;
    ...
    body.innerHTML = model.step === 'setup' ? setupHtml() : model.step === 'manual' ? manualHtml() : model.step === 'scanning' ? scanningHtml() : model.step === 'review' ? reviewHtml() : model.step === 'affected' ? affectedHtml() : savedHtml();
    ...
    next.textContent = model.step === 'affected' ? (model.busy ? 'Saving…' : 'Save room') : model.step === 'review' ? 'Choose affected areas' : 'Review room';
  ...
  function bindBodyEvents() {
    $$('[data-capture-action]').forEach(button => button.addEventListener('click', () => {
      ...
      if (action === 'scan') { model.room = blankRoom(); startNativeCapture(); }
  ...
  async function openDialog() {
    ...
    await loadRooms(); render(); loadCapabilities(); replayOutbox().then(loadRooms).then(render);
  ...
  function buildUi() {
    ...
    $('#fm-capture-next')?.addEventListener('click', () => {
      try {
        if (model.step === 'manual') { buildManualRoom($('#fm-capture-manual-form')); setStatus('Room created. Review each wall and opening.', 'good'); render(); }
  ...
  window.RoomFlowCapture = {
    open: openDialog,
    close: closeDialog,
    receiveProgress(envelope) { const text = $('#fm-capture-progress-text'); if (text) text.textContent = envelope.progress?.message || envelope.message || 'Keep moving slowly around the room.'; },
    acceptNativeResult(result) { model.room = normalizeNativeResult(result); model.history.reset(model.room.vertices); model.step = 'review'; render(); },
    estimateQuantities,
    calculateCost,
    state: model,
  };
