...
EGG = ROOT / "deployment" / "pterodactyl" / "egg-floodman-operations-mobile-v4.6.10.json"
BASE_IMAGE = (
...
def main() -> None:
    ...
    if version != "4.6.10":
        raise SystemExit(f"This reviewed packager is fixed to v4.6.10; found {version!r}")
    launcher = LAUNCHER.read_text(encoding="utf-8")
    if "[Floodman Mobile v4.6.10]" not in launcher:
        raise SystemExit("Canonical launcher does not identify Floodman Mobile v4.6.10")
