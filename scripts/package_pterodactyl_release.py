...
EGG = ROOT / "deployment" / "pterodactyl" / "egg-floodman-operations-mobile-v4.6.9.json"
BASE_IMAGE = (
...
def build_installer_script(launcher: str, version: str) -> str:
    sentinel = "FLOODMAN_LAUNCHER_V469"
...
def main() -> None:
    ...
    if version != "4.6.9":
        raise SystemExit(f"This reviewed packager is fixed to v4.6.9; found {version!r}")
    launcher = LAUNCHER.read_text(encoding="utf-8")
    if "[Floodman Mobile v4.6.9]" not in launcher:
        raise SystemExit("Canonical launcher does not identify Floodman Mobile v4.6.9")
