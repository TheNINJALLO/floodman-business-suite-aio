...
```bash
docker build -f containers/derivative/Dockerfile -t floodman-operations:4.6.9 .
```

Complete AIO image:

```bash
docker build -f containers/base-aio/Dockerfile -t floodman-business-suite-aio:4.6.9 .
```
