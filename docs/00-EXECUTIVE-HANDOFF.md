...
The current custom server baseline is v4.7.1. Android is v0.4.0-alpha01 (build 13). iOS remains v0.1.0-alpha03. RoomFlow is pinned to commit `1f97817a52b916875e50cc6380c0d284072b8ce8`.
...
This handoff contains the complete Floodman custom source and current build files, not the live database. Android 0.4.0-alpha01 retains its recorded unsigned compiler/package evidence and iOS alpha03 retains its recorded simulator evidence, but no native device, signing/store, or live v4.7.1 staging pass is claimed.
