...
| 1 | WEB-001 | Inventory and test every custom web route and interaction surface | NOT STARTED | Inspect Office, PWA, Hub, public customer/signing/payment, Full ERP, and RoomFlow UI sources; add route/render/interaction evidence. |
| 1 | WEB-002 | Repair usability, responsive layout, stacking, and close behavior found by WEB-001 | NOT STARTED | Begins from recorded WEB-001 findings. No broad visual rewrite without preserving behavior. |
...
| 5 | SRV-001 | Run and repair local server smoke suites | NOT STARTED | Use isolated temporary data only; cover routing, Mobile API, PDFs, RoomFlow, workspaces, desktop, and mobile. |
