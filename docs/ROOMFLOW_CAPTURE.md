...
Current source identity is Floodman `4.7.1`, Mobile API `0.3.0-alpha11`, Android `0.4.0-alpha01` (build 13), iOS `0.1.0-alpha03`, and RoomFlow commit `1f97817a52b916875e50cc6380c0d284072b8ce8`. The v4.7.1 change is server-only; native identities, minimum clients, and the exact RoomFlow pin remain unchanged. Physical-device, staging, advisory, signing/store, backup/restore, and production gates remain external.
...
- **Release packaging:** use the matched v4.7.1 runtime, launcher, egg, and checksums; never combine them with or overwrite the frozen v4.7.0 artifacts. Complete device, staging, advisory, clean upgrade/restart, backup/restore, signing/store, and production gates before wider release.
